...
class Patients {
    ...
    /**
     * methode permettant d'afficher la liste des patients de la table patients.
     * @return array
     */
    public function listPatients() {

        $query = 'SELECT * FROM `patients`(`lastname`, `firstname`, `birthdate`, `phone`, `mail`) WHERE `id` = 1 ORDER BY DESC';
        $queryResult = $this->db->prepare($query);
        $queryResult->bindValue(':lastname', $this->lastname, PDO::PARAM_STR);
        $queryResult->bindValue(':firstname', $this->firstname, PDO::PARAM_STR);
        $queryResult->bindValue(':birthdate', $this->birthdate, PDO::PARAM_STR);
